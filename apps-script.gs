...
/* ===== להריץ פעם אחת מהעורך =====
 * בוחרים "setup" בתפריט שליד Run, לוחצים Run, ומאשרים.
 *
 * למה זה קיים: חלון ההרשאות של Deploy נוטה ליפול עם
 * "503 · Service error -27" כששני חשבונות גוגל מחוברים באותו דפדפן.
 * תהליך ההרשאה מתוך העורך יציב יותר, ואחריו הפריסה עוברת בלי לבקש
 * שוב. בונוס: הגיליון נבנה מיד ואפשר לראות שהתבנית יצאה נכון, עוד
 * לפני שהאפליקציה מחוברת. */
function setup() {
  var sh = sheet_();
  summary_(sh.getParent());
  var msg = 'מוכן: ' + sh.getParent().getName() + ' · לשוניות: ' + TAB + ', ' + SUM;
  Logger.log(msg);
  return msg;
}
